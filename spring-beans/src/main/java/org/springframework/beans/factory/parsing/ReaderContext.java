 ...
 *
 * 在Bean定义阅读的进程中传入上下文信息。封装了所有相关的配置和状态。
 *
 * @author Rob Harrop
...
public class ReaderContext {
	...
	/**
	 *  SPI接口（串行外设接口）允许工具和其他外部流程来处理解析Bean定义的过程中报告的错误和警告。
	 */
	private final ProblemReporter problemReporter;

	/**
	 * 在Bean定义的读取注册的过程中，用于接收component、alias和import读取完成后，进行回调的接口
	 */
	private final ReaderEventListener eventListener;

	/**
	 * 配置解析器可以在解析过程中提供附加meta数据。
	 */
