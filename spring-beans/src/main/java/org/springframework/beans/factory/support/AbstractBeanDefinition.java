...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 用来表示一个bean实例化，依赖另一个bean先实例化，对应depend-on属性
	 */
	private String[] dependsOn;

	/**
	 * 为false时，容器在查找自动装配对象时，将不考虑改bean，即它不会被考虑作为其他bean自动装配的候选者，
	 * 但是该bean本身还是可以使用自动装配来注入其他的bean的。
	 * 对应属性：autowire-candidate
	 */
	private boolean autowireCandidate = true;

	/**
	 * 自动装配出现错咯候选者的时候，将作为首选
	 */
	...
	/**
	 * 是否以一种宽松的模式解析构造函数，默认为true，如果为false，则有如下的情况报错
	 * interface ITest();
	 ...
	 *     Main(ITestImpl i){}
	 * }
	 * 抛出异常，因为spring无法确定定位哪个构造函数
	 * 程序设置
	 */
	private boolean lenientConstructorResolution = true;

	/**
	 * 记录constructor-args参数
	 */
	...
	/**
	 * 方法重写属性，记录lookup-method、replaced-method元素
	 */
