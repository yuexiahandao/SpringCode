...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	// 默认使用了DefaultDocumentLoader类来加载Document
	 ...
	 *
	 * 这里的registry其实就是一个BeanFactory，比如XMLBeanFactory
	 */
	 ...
	 * if none specified.
	 * EntityResolver的作用是项目本身就可以提供一个如何寻找DTD声明文件的方法，即由程序来实现寻找DTD声明的过程
	 */
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			...
			try {
				...
				// 以流的方式再来调用另一个doLoadBeanDefinitions方法。
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {
			// 调用doLoadDocument方法使用DefaultDocumentLoader从Resource中获取Document文档。
			// 这其中包括获取对xml的验证模式，加载xml文件，并得到对应的Document。
	 ...
	 * 自动检测就是查看文档是否有DOCTYPE字样，有就是DTD文档，否则就是XSD文档！
	 */
	 ...
	 *
	 * 、、 private Class<?> documentReaderClass = DefaultBeanDefinitionDocumentReader.class;
	 * DefaultBeanDefinitionDocumentReader实例的创建
	 */
	 ...
	 * 这个资源的解析，这里需要传入resource（xml资源）、problemReporter（错误处理）、eventListener（监听器，用于xml解析过程的事件通知等）、
	 * sourceExtractor（预处理meta数据）、NamespaceHandlerResolver（用于从XML的NameSpaceURI中获取对应的处理Handler，
	 * 可以用于定义自定义标签的Handler），这些属性有相应的设置函数。
	 */
	 ...
	 * 延创建NamespaceHandlerResolver
	 * @see #createDefaultNamespaceHandlerResolver()
	 * 这里是需要定义文件加载的路径的，分为xml文档和其它文档的加载，写入系统属性中，所以我们可以在java启动的时候，直接写入
	 * 系统属性中，还有就是xml文档中的#{user.name}也是这样被加载进来的，先加载properties文件进入系统属性中，然后在读取出来。
	 * 所以我们也可以在这里指定这个资源文件的加载路径。
	 */
	 ...
	 *
	 * 创建实现NamespaceHandlerResolver的实现，如果没有设置，那么DefaultNamespaceHandlerResolver就是默认的实现
	 */
