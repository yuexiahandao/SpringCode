 ...
 * 方便实用方法，用来加载Properties属性，执行标准输入流的处理。
 *
 ...
 *
 * 为了更多可配置属性的加载（包括自定义编码的选项），
 * 这时可以考虑使用PropertiesLoaderSupport这个类
 *
...
public abstract class PropertiesLoaderUtils {
	 ...
	 * @throws IOException if loading failed
	 *
	 * 从给定的路径下读取相应的配置文件，可以是xml或者是properties格式的文件。
	 */
	public static Properties loadAllProperties(String resourceName, ClassLoader classLoader) throws IOException {
		// 检查数据，如果classloader为空，使用默认的classloader
		Assert.notNull(resourceName, "Resource name must not be null");
		ClassLoader classLoaderToUse = classLoader;
		// 如果classLoader是空的，那么找一个可用的类加载器
		...
		while (urls.hasMoreElements()) {
			...
			try {
				// 如果是XML文件，那么从xml中加载，否则直接加载属性信息，所有信息写到系统的属性中
